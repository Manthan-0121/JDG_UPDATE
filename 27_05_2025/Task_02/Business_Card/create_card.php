...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Process web links (assuming they're sent as arrays)
    $web_links = [];
    if (!empty($_POST['platform_links'])) {
        foreach ($_POST['platform_links'] as $key => $title) {
            $url = $_POST['platform_links'][$key] ?? '';
    ...
    //     $social_stmt->bindParam(':business_info_id', var: '28');
    ...
    //             if (move_uploaded_file($tmp_name, $destination)) {
    //                 $media_stmt->bindParam(':image', $destination, PDO::PARAM_STR);
    ...
    //     $web_stmt->bindParam(':link_sub_title', '', PDO::PARAM_STR); // Assuming no subtitle
