...
function updateSocialLinks() {
    const platforms = ["whatsapp", "linkedin", "instagram", "twitter", "youtube", "skype"];
    platforms.forEach(platform => {
        const input = document.getElementById(platform).value.trim();
        const link = document.getElementById("link_" + platform);
        if (link) {
            link.href = input || "#";
        }
    });

    const googleMapInput = document.getElementById("googlemap").value.trim();
    const directionBtn = document.getElementById("direction_btn");

    directionBtn.href = googleMapInput || "#";
}
